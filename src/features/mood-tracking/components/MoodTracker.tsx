...
export function MoodTracker() {
  ...
  const generateInsights = () => {
    ...
    const consistentTime = recentMoods.every((mood, i, arr) => {
      if (i === 0) return true;
      const prevTime = new Date(arr[i-1].timestamp).getHours();
      const currentTime = new Date(mood.timestamp).getHours();
  ...
  const handleMoodSelect = (value: number) => {
    ...
    const mood: Mood = {
      value,
      label: moods.find((m) => m.value === value)?.label || '',
      timestamp: new Date(),
    };
    dispatch({ type: 'ADD_MOOD', payload: mood });
  };

  const chartData = state.moods.map((mood) => ({
    date: format(new Date(mood.timestamp), 'MMM d'),
    value: mood.value,
  }));
